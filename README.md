...
2. **Install all required dependencies**
 - Install NextAuth: `pnpm add next-auth@4 @next-auth/prisma-adapter superjson`
...
### Prisma schema for NextAuth incl RBAC

user
```
// Define the Role enum at the top of your schema file
enum Role {
  GUEST
  USER
  ADMIN
...
model User {
  ...
  role          Role      @default(GUEST)  // Default role is USER
  accounts      Account[]
  sessions      Session[]
...
```
model Session {
  id           String   @id @default(cuid())  // ← REQUIRED - internal primary key
  sessionToken String   @unique               // ← The real session identifier
  userId       String
  expires      DateTime
  user         User     @relation(fields: [userId], references: [id], onDelete: Cascade)

  @@index([sessionToken])  // Optional but recommended for performance
}
```
